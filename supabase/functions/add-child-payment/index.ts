...
const logStep = (step: string, details?: any) => {
  ...
  console.log(`[ADD-CHILD-PAYMENT] ${step}${detailsStr}`);
};
...
serve(async (req) => {
  ...
  try {
    ...
    const interval = billingInterval || 'month'; // Default til månedlig
    logStep("Request data", { billingInterval: interval });

    const stripe = new Stripe(Deno.env.get("STRIPE_SECRET_KEY") || "", {
      apiVersion: "2023-10-16",
    });

    // Find eksisterende Stripe customer

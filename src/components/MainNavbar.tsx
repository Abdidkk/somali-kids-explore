...
const highlightClass =
  "relative px-3 py-2 text-sm font-semibold rounded transition-all " +
  "text-white bg-[#4CA6FE] hover:bg-[#1575ad] hover:scale-105 shadow ring-1 ring-[#4CA6FE]/60 hover:ring-[#4CA6FE]";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16 relative">
          ...
          <nav className="hidden md:flex items-center space-x-4">
            {navLinks.map((link, i) => (
              <Link
                to={link.to}
                key={link.label}
                className={highlightClass + " hover-scale"}
                style={{
                  marginLeft: i > 0 ? 6 : 0,
                }}
              >
        ...
        <div className="flex items-center">
          <div className="hidden md:flex items-center space-x-3">
            <Button
              variant="outline"
              className="border-gray-300 hover:border-[#4CA6FE] hover:text-[#4CA6FE]"
            >
              Log Ind
            </Button>
            <Button className="bg-[#4CA6FE] hover:bg-[#1575ad]">
              Tilmeld
            </Button>
          </div>
          ...
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            {navLinks.map((link) => (
              <Link
                ...
                className={
                  highlightClass +
                  " block w-full text-center mb-2 shadow-sm"
                }

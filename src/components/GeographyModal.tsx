...
import GeographyListenActivity from "./GeographyListenActivity";
import GeographyDragDropActivity from "./GeographyDragDropActivity";
...
const GeographyModal: React.FC<GeographyModalProps> = ({
  open,
  onClose
}) => {
          ...
          <div className="flex flex-col gap-6 md:gap-8 py-6 md:py-8 px-3 md:px-4 max-w-3xl mx-auto">
            ...
            >
              ...
              <div>
                ...
                <p className={`${isMobile ? 'text-lg' : 'text-xl'} text-gray-600`}>Klik på kontinenter, lande eller naturtyper og hør navnene på somalisk</p>
              </div>
            </button>
            ...
            >
              ...
              <div>
                ...
                <p className={`${isMobile ? 'text-lg' : 'text-xl'} text-gray-600`}>Træk flag til de rigtige lande eller placer kontinenter korrekt</p>
              </div>
            </button>
          </div>
        ) : activity === "listen" ? (
          <GeographyListenActivity onBack={handleBackToMenu} />
        ) : (
          <GeographyDragDropActivity onBack={handleBackToMenu} />
        )}
      </div>
    </div>
  );

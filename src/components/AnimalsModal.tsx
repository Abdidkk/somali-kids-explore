...
import AnimalsListenActivity from "./AnimalsListenActivity";
import AnimalsGuessActivity from "./AnimalsGuessActivity";
import AnimalsQuizActivity from "./AnimalsQuizActivity";
...
const AnimalsModal: React.FC<AnimalsModalProps> = ({
  ...
  return (
    <div className="fixed z-50 inset-0 flex items-start md:items-center justify-center bg-black/20 backdrop-blur-sm overflow-y-auto pt-[65rem] md:py-10">
  <div className="bg-white rounded-xl shadow-xl px-4 py-5 md:px-7 md:py-6 w-full max-w-sm md:max-w-2xl relative animate-in fade-in-50 mx-2 md:mx-4">
        ...
        <h2 className={`${isMobile ? 'text-xl' : 'text-2xl'} font-semibold text-green-700 mb-3 md:mb-5 text-center pt-3`}>
          Dyr på somalisk
        </h2>
          ...
          <div className="flex flex-col gap-6 md:gap-8 py-6 md:py-8 px-3 md:px-4 max-w-2xl mx-auto">
            ...
            >
              <div className="bg-green-600 text-white p-4 md:p-5 rounded-full flex items-center justify-center min-w-28 min-h-28 md:min-w-32 md:min-h-32">
                <img src="/kategorier/dyr1.png" alt="Lyt og lær" className="w-20 h-20 md:w-25 md:h-25 rounded-full object-cover"/>
              </div>
              <div>
                <h4 className={`font-bold text-green-700 ${isMobile ? 'text-2xl' : 'text-3xl'} mb-2`}>Lyt og lær</h4>
                <p className={`${isMobile ? 'text-lg' : 'text-xl'} text-gray-600`}>Klik på dyrene og hør, hvad de hedder på somalisk</p>
              </div>
            </button>
            ...
            >
              <div className="bg-blue-600 text-white p-4 md:p-5 rounded-full flex items-center justify-center min-w-28 min-h-28 md:min-w-32 md:min-h-32">
                <img src="/kategorier/dyr2.png" alt="Hør og vælg" className="w-20 h-20 md:w-25 md:h-25 rounded-full object-cover" />
              </div>
              <div>
                <h4 className={`font-bold text-blue-700 ${isMobile ? 'text-2xl' : 'text-3xl'} mb-2`}>Hør og vælg</h4>
                <p className={`${isMobile ? 'text-lg' : 'text-xl'} text-gray-600`}>Hør et dyrenavn på somalisk og klik på det rigtige billede</p>
              </div>
            </button>
            ...
            >
              <div className="bg-purple-600 text-white p-4 md:p-5 rounded-full flex items-center justify-center min-w-28 min-h-28 md:min-w-32 md:min-h-32">
                <img src="/kategorier/dyr3.png" alt="Test din viden" className="w-20 h-20 md:w-25 md:h-25 rounded-full object-cover" />
              </div>
              <div>
                <h4 className={`font-bold text-purple-700 ${isMobile ? 'text-2xl' : 'text-3xl'} mb-2`}>Test din viden</h4>
                <p className={`${isMobile ? 'text-lg' : 'text-xl'} text-gray-600`}>Hør og se et dyr, vælg derefter det rigtige blandt tre billeder</p>
              </div>
            </button>
          </div>
        ) : activity === "listen" ? (
          <AnimalsListenActivity onBack={handleBackToMenu} />
        ) : activity === "guess" ? (

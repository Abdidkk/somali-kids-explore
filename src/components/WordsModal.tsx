...
type ActivityType = "listen" | "unjumble" | "quiz" | null;

const WordsModal: React.FC<WordsModalProps> = ({
  open,
  onClose
}) => {
    ...
    <div className="fixed z-50 inset-0 flex items-center justify-center bg-black/20 backdrop-blur-sm overflow-y-auto py-6 md:py-10">
      <div className={`bg-white rounded-xl shadow-xl ${isMobile ? 'px-4 py-5' : 'px-7 py-6'} w-full max-w-2xl relative animate-in fade-in-50 my-auto mx-2 md:mx-4`}>
        ...
        {activity === null ? (
          <div className="flex flex-col gap-6 md:gap-8 py-6 md:py-8 px-3 md:px-4 max-w-2xl mx-auto">
            ...
            >
              ...
              <div>
                ...
                <p className={`${isMobile ? 'text-lg' : 'text-xl'} text-gray-600`}>Hør somaliske ord og lær deres betydning</p>
              </div>
            </button>
            ...
            >
              <div className="bg-green-600 text-white p-4 md:p-5 rounded-full flex items-center justify-center min-w-28 min-h-28 md:min-w-32 md:min-h-32">
                <img src="/lovable-uploads/1e80efba-0e83-48c7-aa88-40fa3c48f0a9.png" alt="Sammensæt sætning" className="w-20 h-20 md:w-24 md:h-24 object-contain" />
              </div>
              <div>
                <h4 className={`font-bold text-green-700 ${isMobile ? 'text-2xl' : 'text-3xl'} mb-2`}>Sammensæt sætning</h4>
                <p className={`${isMobile ? 'text-lg' : 'text-xl'} text-gray-600`}>Træk ordene i den rigtige rækkefølge</p>
              </div>
            </button>
          ...
          <WordsListenActivity onBack={handleBackToMenu} />
        ) : activity === "unjumble" ? (
          <WordsUnjumbleActivity onBack={handleBackToMenu} />
        ) : (
          <WordsQuizActivity onBack={handleBackToMenu} />
        )}
      </div>
    </div>
  );
